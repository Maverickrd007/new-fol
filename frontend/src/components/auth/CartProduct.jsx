...
export default function CartProduct({ _id, name, images, quantity, price }) {
    ...
    const updateQuantityVal = (quantity) => {
        fetch('http://localhost:8000/api/v2/product/cartproduct/quantity', {
            ...
            body: JSON.stringify({
                email: 'Raghav.duggal.s69@kalvium.community',
                productId: _id,
                quantity,
            }),
        })
	...
	return (
		<div className="h-max w-full p-4 flex justify-between border-b border-neutral-600 bg-neutral-800 rounded-lg text-white">
    <div className="flex flex-col gap-y-2">
        <img
            src={`http://localhost:8000${currentImage}`} // Ensure the URL is correct
            alt={name}
            className="w-32 h-32 object-cover rounded-lg border border-neutral-500"
        />
        <div className="flex flex-row items-center gap-x-2 md:hidden">
            <div
                onClick={handleIncrement}
                className="flex justify-center items-center bg-gray-700 hover:bg-gray-600 active:translate-y-1 p-2 rounded-xl cursor-pointer"
            >
                <IoIosAdd />
            </div>
            <div className="px-5 py-1 text-center bg-gray-900 rounded-xl pointer-events-none">
                {quantityVal}
            </div>
            <div
